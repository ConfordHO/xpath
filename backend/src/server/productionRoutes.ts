...
export function registerProductionRoutes(app: express.Express) {
  ...
  app.get("/api/barcodes/:id/print-label", requireRoles("admin", "receptionist", "technician"), async (req, res) => {
    ...
    res.json({
      ...
      imagePath,
      pdfPath,
      printHtml: `<section class="xpath-label"><strong>OLYVIA</strong><br/><span>${barcode.entityType.toUpperCase()}</span><br/><img alt="Barcode" src="${imagePath}" style="max-width:100%;height:auto"/><br/><code>${barcode.code}</code><br/><small>${template?.name ?? "Default browser label"}</small></section>`,
  ...
  app.get("/api/orders/:id/documents/invoice.pdf", requireRoles("admin", "receptionist", "finance", "doctor"), async (req: AuthRequest, res) => {
    ...
    const pdf = await renderPdf((doc) => {
      ...
      doc.moveDown().fontSize(9).fillColor("#64748b").text("Generated by OLYVIA server-side PDF pipeline.");
    });
    res.setHeader("Content-Type", "application/pdf");
