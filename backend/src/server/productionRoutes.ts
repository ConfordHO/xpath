  ...
  app.get("/api/oss/stack-readiness", requireRoles("admin"), async (_req, res) => {
    res.json({
      ...
      categories: [
        ...
        { category: "Voice dictation", implemented: true, libraries: ["open-source Whisper CLI", "ffmpeg"], productionReady: WHISPER_ENABLED, configured: WHISPER_ENABLED && Boolean(WHISPER_COMMAND), command: WHISPER_COMMAND, model: WHISPER_MODEL, controls: ["authenticated endpoint", "audio-size limits", "audit events", "licensed staff verification"] },
        ...
        { category: "AI", implemented: true, libraries: ["Ollama HTTP API", "OpenAI-compatible chat completions"], productionReady: AI_PROVIDER === "local" || Boolean(AI_API_BASE_URL), provider: AI_PROVIDER, model: AI_MODEL, controls: ["drafting aid only", "no invented findings instruction", "audit trail", "human sign-off before release"] },
      ],
    });
  });
}
