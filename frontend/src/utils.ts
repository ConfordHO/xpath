...
export function reportTrafficLightLabel(status?: Report['trafficLightStatus']) {
  switch (status) {
    case 'green':
      return 'Green: ready for release'
    case 'yellow':
      return 'Yellow: pending QC review'
    default:
      return 'Red: report in progress'
  }
}

export function reportReviewStatusLabel(status?: Report['reviewStatus']) {
  switch (status) {
    case 'pending_second_review':
      return 'Pending second-pathologist review'
    case 'corrections_requested':
      return 'Returned for corrections'
    case 'review_validated':
      return 'Review validated, awaiting finalization'
...
export function reportIsReadyForRelease(report?: Pick<Report, 'releaseRuleStatus' | 'reviewStatus'> | null) {
...
export async function downloadPdfDocument(
  ...
  const footer = options.footer ?? 'OLYVIA by X.PATH Labs'
  pdf.setFontSize(9)
...
const reportCopy = {
  en: {
    ...
    signout: 'SIGN-OUT',
    authenticity:
      'This result was electronically generated by OLYVIA. Scan the QR code or open the validation link to confirm authenticity.',
    validationLink: 'Validation link',
    generatedBy: 'END OF REPORT - Generated by OLYVIA',
  ...
  fr: {
    ...
    signout: 'VALIDATION',
    authenticity:
      "Ce resultat a ete genere electroniquement par OLYVIA. Scannez le QR code ou ouvrez le lien de validation pour verifier son authenticite.",
    validationLink: 'Lien de validation',
    generatedBy: 'FIN DU RAPPORT - Genere par OLYVIA',
...
export async function downloadPathologyReportPdf(
  ...
  y = drawTextBlock(
    ...
    [
      ...
      `QC: ${reportTrafficLightLabel(report?.trafficLightStatus)} (${reportReviewStatusLabel(report?.reviewStatus)})`,
      `Second reviewer: ${report?.secondReviewerName ?? '—'}`,
      `Signed by: ${report?.signedBy ?? '—'}`,
      `Signed at: ${reportDateTime(report?.signedAt, locale)}`,
      `Finalized: ${reportDateTime(report?.finalizedAt, locale)}`,
